...
mod tests {
    ...
    fn test_relative_atomic_mass() {
        for isotope in RubidiumIsotope::iter() {
    ...
    fn test_element() {
        for isotope in RubidiumIsotope::iter() {
            ...
            assert_eq!(element, crate::Element::Rb, "Element should be correct for {isotope:?}");
        }
    }
    #[test]
    fn test_mass_number() {
    ...
    fn test_most_abundant() {
        let most_abundant = RubidiumIsotope::most_abundant_isotope();
    ...
    fn test_from_isotope() {
        for isotope in RubidiumIsotope::iter() {
            ...
            match iso {
                crate::Isotope::Rb(i) => assert_eq!(i, isotope),
    ...
    fn test_from_element() {
        for isotope in RubidiumIsotope::iter() {
            ...
            assert_eq!(elem, crate::Element::Rb);
        }
    }
    #[test]
    fn test_try_from_mass_number() {
        for isotope in RubidiumIsotope::iter() {
            ...
            let iso_u32 = RubidiumIsotope::try_from(u32::from(mass)).unwrap();
            assert_eq!(iso_u32, isotope);
            if let Ok(mass_u8) = u8::try_from(mass) {
                let iso_u8 = RubidiumIsotope::try_from(mass_u8).unwrap();
                assert_eq!(iso_u8, isotope);
            }
        }
        assert!(RubidiumIsotope::try_from(0_u16).is_err());
        assert!(RubidiumIsotope::try_from(1000_u16).is_err());
        assert!(RubidiumIsotope::try_from(0_u32).is_err());
        assert!(RubidiumIsotope::try_from(1000_u32).is_err());
        assert!(RubidiumIsotope::try_from(0_u8).is_err());
    }
    #[test]
    fn test_display() {
        for isotope in RubidiumIsotope::iter() {
