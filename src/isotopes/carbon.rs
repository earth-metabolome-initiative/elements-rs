...
mod tests {
    ...
    fn test_try_from_mass_number() {
        for isotope in CarbonIsotope::iter() {
            ...
            let iso_u32 = CarbonIsotope::try_from(u32::from(mass)).unwrap();
            assert_eq!(iso_u32, isotope);
            if let Ok(mass_u8) = u8::try_from(mass) {
                let iso_u8 = CarbonIsotope::try_from(mass_u8).unwrap();
                assert_eq!(iso_u8, isotope);
            }
        }
        assert!(CarbonIsotope::try_from(0_u16).is_err());
        assert!(CarbonIsotope::try_from(1000_u16).is_err());
        assert!(CarbonIsotope::try_from(0_u32).is_err());
        assert!(CarbonIsotope::try_from(1000_u32).is_err());
        assert!(CarbonIsotope::try_from(0_u8).is_err());
    }
    #[test]
    fn test_display() {
        for isotope in CarbonIsotope::iter() {
