...
impl core::fmt::Display for IridiumIsotope {
...
mod tests {
    ...
    fn test_relative_atomic_mass() {
        for isotope in IridiumIsotope::iter() {
    ...
    fn test_element() {
        for isotope in IridiumIsotope::iter() {
            ...
            assert_eq!(element, crate::Element::Ir, "Element should be correct for {isotope:?}");
        }
    }
    #[test]
    fn test_mass_number() {
    ...
    fn test_most_abundant() {
        let most_abundant = IridiumIsotope::most_abundant_isotope();
    ...
    fn test_from_isotope() {
        for isotope in IridiumIsotope::iter() {
            ...
            match iso {
                crate::Isotope::Ir(i) => assert_eq!(i, isotope),
    ...
    fn test_from_element() {
        for isotope in IridiumIsotope::iter() {
            ...
            assert_eq!(elem, crate::Element::Ir);
        }
    }
    #[test]
    fn test_try_from_mass_number() {
        for isotope in IridiumIsotope::iter() {
            ...
            let iso_u32 = IridiumIsotope::try_from(u32::from(mass)).unwrap();
            assert_eq!(iso_u32, isotope);
            if let Ok(mass_u8) = u8::try_from(mass) {
                let iso_u8 = IridiumIsotope::try_from(mass_u8).unwrap();
                assert_eq!(iso_u8, isotope);
            }
        }
        assert!(IridiumIsotope::try_from(0_u16).is_err());
        assert!(IridiumIsotope::try_from(1000_u16).is_err());
        assert!(IridiumIsotope::try_from(0_u32).is_err());
        assert!(IridiumIsotope::try_from(1000_u32).is_err());
        assert!(IridiumIsotope::try_from(0_u8).is_err());
    }
    #[test]
    fn test_display() {
        for isotope in IridiumIsotope::iter() {
